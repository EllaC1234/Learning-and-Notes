    <footer>
        <div class="box-container">
            <div class="box-3 flex ">
                <ul>
                    <!-- <li><a href="<?php echo site_url();?>" class="link">Home</a></li>
                    <li><a href="<?php echo site_url('/about-us');?>" class="link">About</a></li>
                    <li><a href="#" class="link">Courses</a></li>
                    <li><a href="<?php echo site_url('/contact-us');?>" class="link">Contact</a></li> -->
                <?php 
                $args = array(
                    'theme_location' => 'footer-menu'
                );

                wp_nav_menu($args);
                ?>
                </ul>
                <ul>
                    <li><a href="#" class="link">Legal</a></li>
                    <li><a href="#" class="link">FAQS</a></li>
                    <li><a href="#" class="link">Privacy Policy</a></li>
                    <li><a href="#" class="link">Careers</a></li>
                </ul>
            </div>
            <div class="box-3 ">
                <div class="subs">
                <div>
                    <form action="">
                    <input type="email" name="" placeholder="Enter your email" id="" class="email" />
                    </form>
                </div>
                <div>
                    <input type="submit" value="Subscribe" class="btn" />
                </div>
                </div>
                <p>Subscribe for latest upadates</p>
                <div class="share">
                    <a href="#" class="fab fa-facebook-square fa-2x"></a>
                    <a href="#" class="fab fa-twitter-square fa-2x"></a>
                    <a href="#" class="fab fa-instagram-square fa-2x"></a>
                    <a href="#" class="fab fa-linkedin fa-2x"></a>
                </div>
            </div>
        </div>
        <div class="credit">&copy; DataJek | All rights reserved.</div>
    </footer>

    <?php
    wp_footer();
    ?>
</body>
</html>