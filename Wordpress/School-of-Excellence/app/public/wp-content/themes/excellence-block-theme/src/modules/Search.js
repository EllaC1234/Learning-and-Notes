class Search{
    ...
    sendSearchRequest(){
        console.log(siteData.root_url+ '/wp-json/excellence/v1/search?term='+this.searchInput.value);
        fetch(siteData.root_url+ '/wp-json/excellence/v1/search?term='+this.searchInput.value)
        .then((response)=>{
            return response.json();
        })
        .then((data)=>{
            
            if(data.posts.length|| data.pages.length|| data.courses.length|| data.teachers.length|| data.events.length){
                this.searchResults.innerHTML =`
                <div class="results-container">
                    <div class="results-column">
                        <h3 class="search-results-title">Blog Posts</h3>
                        ${data.posts.length ? '<ul class = "list-item">' : '<p>No posts matched your search</p>'}
                        ...
                        ${data.pages.length ? '<ul class = "list-item"> ' : '<p>No pages matched your search</p>'}
                        ...
                        <h3 class="search-results-title">Courses</h3>
                            ${data.courses.length ? '<ul class = "list-item">' : '<p>No courses matched your search</p>'}
                    ...
                    <div class="results-column">
                        <h3 class="search-results-title">Teachers</h3>
                            ${data.teachers.length ? '<ul class = "results-page-flex">' : "<p>No teachers matched your search.</p>"}
                                ${data.teachers.map(element => `
                        ...
                        <h3 class="search-results-title">Events</h3>
                            ${data.events.length ? '' : '<p>No events matched your search</p>'}
