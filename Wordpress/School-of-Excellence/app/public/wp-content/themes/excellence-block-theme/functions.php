...
class ExcellenceBlock {
    ...
    function __construct( $name, $imageurl = null ){
        $this->name = $name;
    ...
    function excellence_block_init(){
        ...
        wp_register_script($this->name, get_stylesheet_directory_uri().'/build/custom-blocks.js', array('wp-blocks', 'wp-editor'));
        if($this->imageurl){
            wp_localize_script($this->name, $this->name, $this->imageurl);
        }
        
        register_block_type_from_metadata(__DIR__."/blocks/".$this->name."/block.json", array(
...
new ExcellenceBlock("button");
new ExcellenceBlock("banner", ['fallbackImage' => get_theme_file_uri('/assets/images/banner-main.jpg')]);